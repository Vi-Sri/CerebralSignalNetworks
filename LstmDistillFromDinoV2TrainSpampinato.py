...
class FeatureDistributionLoss(nn.Module):
    ...
    def forward(self, student_outputs, teacher_outputs, epoch, label):
        overall_loss = 0
        ...
        ce_loss = nn.functional.cross_entropy(student_outputs, label)
...
if __name__=="__main__":
    ...
    LSTM_model = Model(input_size=128,lstm_size=128,lstm_layers=4,output_size=features_length, include_top=False)
    if os.path.exists(FLAGS.custom_model_weights):
        LSTM_model.load_state_dict(torch.load(FLAGS.custom_model_weights))
    ...
    opt = torch.optim.AdamW(lr=learning_rate, params=LSTM_model.parameters())
    ...
    for EPOCH in range(EPOCHS):
        ...
        if EPOCH % validation_frequency==0 and EPOCH>0:
            ...
            if best_val_loss is None:
                ...
                torch.save(LSTM_model.state_dict(), f"{FLAGS.log_dir}/lstm_dinov2_epoch_{EPOCH}_best_loss.pth")
            else:
