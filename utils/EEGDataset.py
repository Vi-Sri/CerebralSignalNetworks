...
class EEGDataset(Dataset):
    ...
    def __init__(self, eeg_signals_path, 
        ...
        # print(f"time low: {time_low} time high : {time_high}")
        ...
        try:
            ...
            self.eeg_means  = stddevsfile_loaded["means"]
        ...
        for sub_idx in subset_indexes:
            ...
            #     # print("Original ", loaded['dataset'][sub_idx]["eeg"].size())
            #     tempEEG = loaded['dataset'][sub_idx]["eeg"].float().t()
            #     tempEEG = tempEEG.cpu().numpy().T
            #     for ch_idx in range(tempEEG.shape[-1]):
            #         tempEEG = self.normlizeEEG(EEG=tempEEG,ch_index=ch_idx,class_index=None)
                
            #     loaded['dataset'][sub_idx]["eeg"] = torch.from_numpy(tempEEG)
            #     # print("after ", loaded['dataset'][sub_idx]["eeg"].size())
    ...
    def ExtractImageFeatures(self, preprocsessor, model, device):
        ...
        logger.info("Extracting Image features done")
        self.image_features_extracted = True
